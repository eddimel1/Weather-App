...
export const VideoPlayerSeekBar = (props: propsType) => {
    ...
    function calcLastBufferingPosition () {
        if(player && player.buffered.length){
            return player.buffered.start((player.buffered.length -1)) / player.duration * 100 
        ...
        if(player &&  seekBarRef.current && props.playerState ==='playing'){
            interval = setInterval(()=>{
                if(player){
                    ...
                    setBufferingWidth((100 - bufferingPosition.current) * (invlerp(player.buffered.start(player.buffered.length -1),player.duration,player.buffered.end(player.buffered.length -1))))
                   }
            },10)
  ...
  return (
    <div className={classes.seekBar} ref={seekBarRef} style={{ zIndex: '500' }} onClick={(e: React.MouseEvent) => { setProgressWidth(calcPersentage(e.nativeEvent.offsetX , seekBarRef.current?.offsetWidth!))
        if(player && seekBarRef.current){

            player.currentTime = player.duration * calcPersentage(e.nativeEvent.offsetX ,seekBarRef.current.offsetWidth) / 100
          ...
          <div  className={classes.progressBar} style={{ width: `${progressWidth}%` }}></div>
          <div className={classes.bufferingBar} style={{width: `${bufferingWidth}%` , left:`calc(${bufferingPosition}% - 15px)`}}></div>
      </div>
  )
}
