...
const Buttons = ({setMode,mode} :{setMode : React.Dispatch<React.SetStateAction<mode>> , mode:string})  => {
  ...
  return (
    ...
    <div className = {classes.container}>
        ...
        <img src={logo} className={classes.logo} alt="logo"  style={{borderRadius:'15px' , border:'2px solid violet' ,maxHeight:'80px'}} />
        ...
        <div className={classes.list} ref={listRef}>
            {someData.map((button : objType, i : number) => {
             ...
             return <NavItem setToggle={setToggle} toggle={toggle} setSelected={setSelected} selected={selected } isSelected={Number(selected) === i} listRef={listRef} text={button.title}  ind={i} key={Date.now() +i} > {button.icon}  </NavItem> 
