...
export const NavItem: FC<buttonProps> = (props) => {
  ...
  const toggleDropDown = (event: MouseEvent) => {
    if (
      listRef.current &&
      event.target &&
      !listRef.current.contains(event.target as HTMLUListElement) &&
      !props.listRef.current?.contains(event.target as HTMLUListElement)
    ) {
      ...
      <div className={`${classes.container}  `}>
        <div id="div">
          <div
            className={`${classes.Item} ${
              props.isSelected ? classes.matchButtonStyle : ''
            } `}
        ...
        {props.isSelected && props.toggle && (
          <ul className={classes.hiddenList} ref={listRef}>
            {someData[Number(props.selected)].subDropDowns.map(
              (item: subDropDownsType, i: number): any => {
                return (
                  <SubDropDown
                    ...
                    icon={item.icon}
                    index={i}
                    clickHandler={subDropDownHandler}
