...
export const QuestionScreen = (props: propType) => {
  ...
  const returnAnswersStyle = (
    usersAnswer: string,
    answer: string,
    array: quizzAnswerData
  ) => {
  ...
  useEffect(() => {
    props.state &&
      fetch(
        `https://the-trivia-api.com/api/questions?categories=${props.state.category}&difficulty=${props.state.difficulty}&limit=10`
  ...
  useEffect(() => {
    if (data && !questionAnswered) {
      ...
      const mergedArray =
        incorrectAnswers && incorrectAnswers.concat(correctAnswer)
    ...
    <div className={classes.questionScreenWrapper}>
      <div className={classes.questionScreenContainer}>
        <div className={classes.topContainer}>
          <div className={classes.quizzCategory}>
            Category : {data && data[portionIndex].category}
          </div>
          <div className={classes.questionCount}>{portionIndex}/10</div>
          <div className={classes.difficulty}>
            Difficulty : {data && data[portionIndex].difficulty}
          </div>
        </div>
        <div className={classes.question}>
          {data && data[portionIndex].question}
        </div>
            ...
            allAnswers.map((answer, i) => {
              return (
                <button
                  ...
                  key={Date.now() + i}
                  onClick={() =>
                    data && checkAnswer(answer, data[portionIndex])
                  }
                  className={`${classes.questionItem} ${
                    data &&
                    usersAnswer &&
                    questionAnswered &&
                    returnAnswersStyle(usersAnswer, answer, data[portionIndex])
                  }`}
                >
                  {' '}
                  {answer}
        ...
        <div className={classes.quizzBottom}>
          <button
            ...
            onClick={() => {
              setPortionIndex((prev) => prev + 1)
              setQuestionAnswered(false)
              props.dispatch({type: 'nextQuestion'})
            }}
          >
            Next Question
          </button>
