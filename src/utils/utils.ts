...
export function toSunrise (unix : number | undefined ): any {
 ...
 export function hasProperty <T extends object>  (prop : string , obj : T | undefined ) : string | undefined  {
  ...
  export async function api<T>(url: string): Promise<T> {
    ...
    try {
        const response = await fetch(url as RequestInfo ,{method:'get'} as RequestInit)
        ...
        export const getLatAndLon = (town:string) : void | Promise<Array<number>>  =>   {
            return new Promise(async (res,rej)=> {
                    ...
                    const cords = await api<geoCodingObjectType>(`http://api.openweathermap.org/geo/1.0/direct?q=${town}&limit=1&appid=${apiKey}`)
       ...
       export  function getDatefromData(data : 'month' | 'day' | 'dayOfMonth' | 'hours' | 'minutes')  {
            switch (data) {
                case 'month':
...
export function addComaToArrayOfStrings(array : Array<string> | undefined) :string | undefined {
